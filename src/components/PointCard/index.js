...
const PointCard = ({ id, photoUrl, type, location, price, Likes, unlike, isOwner }) => {
  ...
  const like = () => {
    ...
    requester(`POST`, `${process.env.REACT_APP_API_BACKEND_URL}/points/like`, { pointId: id }, {}, history)
  ...
  const viewHouse = () => history.push(`/view-point/${id}`)

  const updateHouse = () => history.push(`/update-point/${id}`)

  return (
    <div className="main-card">
      <div className="card-content">
        <div className="card-content-header" onClick={isOwner ? updateHouse : viewHouse}>
          <img alt="" src={(photoUrl?.length && photoUrl[0]) || require('../../assets/images/point-default.jpg')} />
        </div>
